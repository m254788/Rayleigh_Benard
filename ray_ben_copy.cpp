...
int main(int argc, char* argv[]) {
	...
	for(int s = 0; s < 5; s++){
		for(int t = 0; t < lx; t++){
			tIn[s*nnodes+top_nodes[t]] = tT[s]*Tcold;
		}
	}
	int numTopTriggers = 10;
	int numBottomTriggers = 10;
	bool uniformTriggers = true;
	srand((unsigned) time(0));
	
	for(int s = 0; s < 5; s++){ //create asymmetry NOTE: it's cool to add multiple asymmetries		
		for(int t = 1; t < numBottomTriggers+1; t++){
			if (uniformTriggers){
				tIn[s*nnodes+int((1+(double(t)/(numBottomTriggers+1)))*lx)] = tT[s]*(1.1*Thot);
			}
			else {
				tIn[s*nnodes+int((1+(double(rand())/RAND_MAX))*lx)] = tT[s]*(1.1*Thot);	
			}
		}
		for(int t = 1; t < numTopTriggers+1; t++){
			if (uniformTriggers) {
				tIn[s*nnodes+nnodes-int((1+(double(t)/(numTopTriggers+1)))*lx)] = tT[s]*(0.9*Tcold);
			}	
			else {
				tIn[s*nnodes+nnodes-int((1+(double(rand())/RAND_MAX))*lx)] = tT[s]*(0.9*Tcold);
			}	
		}
	}
